...
int main()
{
    ...
    Room room101(conferenceRoom,101);
    ...
    //Students have no access to the lecture rooms
    studentBatyrgariev.goToTheRoom(room107);
    //Admin can give access to any room
    adminFirst.addAccessedRoom(room107, &studentBatyrgariev);
    //And now student can go to this room
    studentBatyrgariev.goToTheRoom(room107);
    ...
    //create some guests
    Guest guest1("Guest","Guestov","Robotics olympiad");
    Guest guest2("Guest2","Guestov2","Projects school");
    //guests can do actions related to their competition(olympiad, project schools or some other competitions)
    guest1.doSomeCompetitionActions();
    guest2.doSomeCompetitionActions();
    //guests have access to the all lectures and conference rooms in 1st floor
    //rooms between 100 and 199
    guest1.goToTheRoom(room107);
    guest2.goToTheRoom(room107);
    //guests have no access to the conference room in the 3rd floor
    guest1.goToTheRoom(room307);
    //guests have access to the conference room in the 1st floor
    guest2.goToTheRoom(room101);
    //admin can give access to any room
    adminSecond.addAccessedRoom(room307,&guest1);
    //now guest can go to that room
    guest1.goToTheRoom(room307);


    //student have no access to the lecture rooms
    studentBatyrgariev.goToTheRoom(room105);
    //we can change emergency status
    changeEmergency(true);
    //and all have access to all rooms but they should go to the exit))))
    studentBatyrgariev.goToTheRoom(room105);
